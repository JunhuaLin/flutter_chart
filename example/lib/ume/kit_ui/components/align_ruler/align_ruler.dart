import 'package:example/ume/core/pluggable.dart';
import 'package:example/ume/core/service/inspector/inspector_overlay.dart';
import 'package:example/ume/core/util/constants.dart';
import 'package:example/ume/kit_ui/components/hit_test.dart';
                ...
                Expanded(
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text('Right: ${(_windowSize.width - _dotPosition.dx).toStringAsFixed(1)}',
                          style: style),
                    ],
                  ),
                ),
                Expanded(
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Padding(padding: const EdgeInsets.only(top: 8)),
                      Text('Bottom: ${(_windowSize.height - _dotPosition.dy).toStringAsFixed(1)}',
                          style: style),
                    ],
                  ),
                ),
              ...
              children: <Widget>[
                Container(
                  ...
                  child: Transform.scale(
                    scale: 1.3,
                    child: Switch(
                        value: _switched, onChanged: _switchChanged, activeColor: Colors.red),
                  ),
                ),
                Expanded(
                  child: Padding(
    ...
    return Container(
      ...
      child: Stack(
        ...
        children: [
          ...
          Positioned(
              left:
                  _dotPosition.dx + (_windowSize.width - _dotPosition.dx) / 2 - _textSize.width / 2,
              top: horizontalTop,
              child: Text('${(_windowSize.width - _dotPosition.dx).toStringAsFixed(1)}',
                  style: _fontStyle)),
          Positioned(
              top: _dotPosition.dy +
                  ...
                  _textSize.height / 2,
              left: verticalLeft,
              child: Text('${(_windowSize.height - _dotPosition.dy).toStringAsFixed(1)}',
                  style: _fontStyle)),
          Positioned(
              ...
              child: Container(
                child: Center(
                  child: Container(
                    ...
                    decoration:
                        BoxDecoration(shape: BoxShape.circle, color: Colors.red.withOpacity(0.8)),
                  ),
                ),
                height: _dotSize.height,
                width: _dotSize.width,
                decoration: BoxDecoration(
                    borderRadius: _radius, border: Border.all(color: Colors.black, width: 2)),
              ),
            ),
          ),
          Positioned(
